...
@interface KLWebViewController ()<WKNavigationDelegate, WKUIDelegate>
...
- (void)_updateFrameOfProgressView {
    ...
    CGFloat barFrameYOnX = isX ? 44.f : 20 ;
...
- (UIBarButtonItem *)backNavLeftItem {
    ...
    if (_backImagePath) {
        ...
        imagePath = [NSString stringWithFormat:@"%@/%@/%@",resourceBundle.resourcePath,@"JSResources.bundle", @"webView_back"];
...
- (void)unregisterSupportProtocolWithHTTP:(BOOL)supportHTTP
                                  schemes:(NSArray<NSString *> *)schemes
                            protocolClass:(Class)protocolClass {
    [KLWebView jxb_unregisterProtocolWithHTTP:supportHTTP
                            customSchemeArray:schemes
                             urlProtocolClass:protocolClass];
}
